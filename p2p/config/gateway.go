...
type Gateway struct {
	...
	// specify in X-Quantos-Path-Prefix header.
	//
	// The X-Quantos-Path-Prefix header is used to specify a base path to prepend
	...
	//
	// Example: To mount blog.quantos.network (a DNSLink site) at quantos.network/blog
	// set PathPrefixes to ["/blog"] and nginx config to translate paths
	// and pass Host header (for DNSLink):
	//  location /blog/ {
	//    rewrite "^/blog(/.*)$" $1 break;
	//    proxy_set_header Host blog.quantos.network;
	//    proxy_set_header X-Quantos-Gateway-Prefix /blog;
